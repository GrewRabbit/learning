...
// 流程图 JSON 生成服务（Stage 2，FR-008，Zod 校验 + 失败重试最多 2 次）
...
} from '@/app/lib/ai/prompts/flowchart-prompt';
...
export class FlowchartService {
  ...
  async generate(input: GenerateInput): Promise<ServiceResult<Flowchart>> {
    try {
      ...
      const baseMessages = buildFlowchartPrompt(input);
      let lastError = '';

      for (let attempt = 0; attempt <= MAX_RETRY_COUNT; attempt++) {
        // 重试时附错误信息让模型修正（NFR-005）
        const currentMessages: ChatMessage[] = lastError
          ? buildFlowchartRetryPrompt(baseMessages, lastError)
          : baseMessages;
        ...
        try {
          ...
          logger.warn('流程图 JSON 解析失败', { attempt, error: lastError });
          continue;
        }

        // Zod 校验
        const validated = FlowchartSchema.safeParse(parsed);
        if (validated.success) {
          ...
          logger.info('流程图生成完成', {
            ...
            edgeCount: validated.data.edges.length,
          });
          return { success: true, data: validated.data };
        }

        lastError = validated.error.message;
        logger.warn('流程图 Zod 校验失败', { attempt, error: lastError });
      }

      // 3 次均失败（JSON 解析或 Zod 校验失败）
      logger.error('流程图生成失败，重试次数耗尽', { maxRetry: MAX_RETRY_COUNT });
      return {
        success: false,
        error: {
      ...
      // LLM 调用本身失败（网络错误等）
      logger.error('流程图生成异常', {
        error: error instanceof Error ? error.message : String(error),
      });
      return {
        success: false,
        error: {
          code: 'CPP_AI_FLOWCHART_GENERATION_FAILED',
          message: '流程图生成失败，可重试',
        },
      };
    }
