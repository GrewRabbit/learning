...
test.describe('API 契约测试 @fast', () => {
  ...
  test('GET /api/solve 缺少 jobId → 400 + GESP6_INPUT_INVALID', async ({ request }) => {
  ...
  test('GET /api/solve 不存在的 jobId → 404 + GESP6_JOB_NOT_FOUND', async ({ request }) => {
    const response = await request.get('/api/solve?jobId=nonexistent-job-id-12345');
...
test.describe('DELETE /api/solve?jobId=xxx（取消任务） @fast', () => {
  ...
  test('DELETE 不存在的 jobId → 404 + GESP6_JOB_NOT_FOUND', async ({ request }) => {
    const response = await request.delete('/api/solve?jobId=nonexistent-job-id-67890');
    expect(response.status()).toBe(404);
    const body = (await response.json()) as SolveErrorResponse;
    expect(body.success).toBe(false);
    expect(body.error.code).toBe('GESP6_JOB_NOT_FOUND');
  ...
  test('POST 创建任务 → DELETE 取消 → GET 返回 GESP6_CANCELLED', async ({ request }) => {
    ...
    const postResponse = await request.post('/api/solve', {
      data: {
        problem: {
          type: 'text',
    ...
    const deleteResponse = await request.delete(`/api/solve?jobId=${jobId}`);
    ...
    const getResponse = await request.get(`/api/solve?jobId=${jobId}`);
  ...
  test('DELETE 已取消的任务 → 404（cancelJob 仅允许 processing → cancelled）', async ({ request }) => {
    ...
    const firstDelete = await request.delete(`/api/solve?jobId=${jobId}`);
    ...
    const secondDelete = await request.delete(`/api/solve?jobId=${jobId}`);
