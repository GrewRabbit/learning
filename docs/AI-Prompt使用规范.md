...
| T19 | 架构设计未评审直接进入开发 | 架构设计必须经过 4.5.2 评审 + 4.5.4 终审，approved 后才可开发 |
| T20 | 架构设计遗漏 spec 的 FR | 架构终审（4.5.4）必须核查 FR 覆盖率 100% |
...
- [ ] **架构闭环**：架构设计包含生成→评审→修订→终审完整流程（4.5.1-4.5.4）
...
| `nextjs-architect` | 架构设计生成 / 架构设计修订 | 4.5.1 / 4.5.3 |
| `general_purpose_task` | 架构设计评审 / 调试排障 / 代码审查 | 4.5.2 / 4.8 / 4.9 |
...
| 总调度 agent（主对话） | Spec 终审 / 架构终审 / 调度编排 | 4.4 / 4.5.4 / 第五章 |
