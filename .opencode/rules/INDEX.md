...
> 本目录为 `.trae/rules/` 的 opencode 副本，两处配置保持一致；原始 `.trae/rules/` 保留不动。
...
| `nextjs-frontend-expert` | `dev/component-rules.md` + `dev/dev-workflow.md` | 前端组件与 UI 规范 |
| `nextjs-backend-expert` | `dev/api-conventions.md` + `dev/dev-workflow.md` | Server Action / API / 认证规范 |
...
| `nextjs-db-modeler` | `dev/api-conventions.md` | 服务层与数据访问规范 |
| `nextjs-prd-generator` | `spec/*` | PRD 生成 |
| `nextjs-prd-reviewer` | `spec/*` | PRD 评审 |
| `nextjs-code-reviewer` | `dev/*` + `global/*` | 代码验收审查 |
| `nextjs-architecture-reviewer` | `spec/*` + `dev/*` | 架构评审 |
| `pencil-ui-designer` | `dev/component-rules.md` | Pencil 设计稿工作 |
